...
export default function Recruitment() {
  ...
  const handleExportData = (format = 'csv') => {
    ...
    if (format === 'pdf') {
      // PDF format - create HTML representation
      const html = `
        <html>
          <head>
          ...
          <body>
            ...
            <div class="summary">
              ...
              <p><strong>Pending/New:</strong> ${filteredApplications.filter(a => ['new', 'pending'].includes(a.status)).length}</p>
              <p><strong>Under Review:</strong> ${filteredApplications.filter(a => ['reviewed', 'review'].includes(a.status)).length}</p>
              <p><strong>Shortlisted/Interview:</strong> ${filteredApplications.filter(a => ['shortlisted', 'interview'].includes(a.status)).length}</p>
              <p><strong>Hired:</strong> ${filteredApplications.filter(a => a.status === 'hired').length}</p>
              <p><strong>Rejected:</strong> ${filteredApplications.filter(a => a.status === 'rejected').length}</p>
            </div>
            ...
            <div class="footer">
              <p>Generated by LiteHR Manager Portal</p>
            </div>
          </body>
        </html>
      `;
      dataStr = html;
      fileName = `recruitment-applications-${new Date().toISOString().split('T')[0]}.html`;
      mimeType = 'text/html';
    } else {
      // CSV format
